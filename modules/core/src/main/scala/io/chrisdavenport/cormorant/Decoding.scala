...
object Decoding {
  ...
  def readRows[A: Read](rows: CSV.Rows): List[Either[Error.DecodeFailure, A]] =
    rows.rows.map(Read[A].read)
  def readComplete[A: Read](complete: CSV.Complete): List[Either[Error.DecodeFailure, A]] =
    readRows(complete.rows)
