...
class TSVParserSpec extends munit.FunSuite {
  ...
  test("parse a single header") {
    ...
    assertEquals(TSVParser.name.parse(basicString), Right(("\t", expect)))
  }
  ...
  test("parse first header in a header list") {
    ...
    assertEquals(TSVParser.name.parse(baseHeader), Right(("\tSomething2\tSomething3", expect)))
  }
  ...
  test("parse a group of headers") {
    ...
    val result = (TSVParser.name, (TSVParser.SEPARATOR *> TSVParser.name).rep0).mapN(_ :: _).parse(baseHeader)
  ...
  test("parse headers correctly") {
    ...
    val result = TSVParser.header.parse(baseHeader)
  ...
  test("parse a row correctly") {
    ...
    assertEquals(TSVParser.record.parse(singleRow), Right(("", expected)))
  }
  ...
  test("parse rows correctly") {
    ...
    assertEquals(TSVParser.fileBody.parse(csvParse), Right(("", csv)))
  }
  ...
  test("parse a complete csv with a trailing new line by stripping it") {
    ...
    assertEquals(TSVParser.`complete-file`
      .parse(expectedCSVString)
      .map(_._2.stripTrailingRow) , Right(csv))
  }
  ...
  test("parse an escaped row with a tab") {
    ...
    assertEquals(TSVParser.record.parse(parseString), Right(("", csv)))
  }
