...
object TSVParser {
  ...
  // COMMA = %x2C
  val TAB: Parser[Char] = char(tab).named("TAB")

  // TEXTDATA =  %x20-21 / %x23-2B / %x2D-7E
  val TEXTDATA: Parser[Char] = noneOf(dquoteS + tabS + crS + lfS).named("TEXTDATA")

  // escaped = DQUOTE *(TEXTDATA / COMMA / CR / LF / 2DQUOTE) DQUOTE
  val escaped: Parser[CSV.Field] =
    (DQUOTE ~> many(TEXTDATA | TAB | CR | LF | TWO_DQUOTE.map(_ => dquote))
      .map(_.mkString)
  ...
  val header: Parser[CSV.Headers] = (name, many(TAB ~> name))
    .mapN(NonEmptyList(_, _))
    .map(CSV.Headers)
    .named("CSV.Headers")

  // record = field *(COMMA field)
  val record: Parser[CSV.Row] = (field, many(TAB ~> field))
    .mapN(NonEmptyList(_, _))
    .map(CSV.Row)
    .named("CSV.Row")
