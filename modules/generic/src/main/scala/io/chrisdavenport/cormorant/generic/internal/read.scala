...
trait ReadProofs extends LowPriorityReadProofs {
  ...
  implicit def readHNil[H](implicit G: Get[H]): Read[H :: HNil] = new Read[H :: HNil] {
    def readPartial(
        a: CSV.Row
    ): Either[Error.DecodeFailure, Either[(CSV.Row, H :: HNil), H :: HNil]] = a match {
  ...
  implicit def hlistRead[H, T <: HList](implicit
      G: Get[H],
      R: Lazy[Read[T]]
        ...
        case CSV.Row(NonEmptyList(h, t)) =>
          (
            ...
            NonEmptyList
              .fromList(t)
              .fold(
                Either.left[Error.DecodeFailure, Either[(CSV.Row, T), T]](
                  Error.DecodeFailure.single("Unexpected End Of Input")
                )
              )(nel => R.value.readPartial(CSV.Row(nel)))
          ).parMapN {
...
private[internal] trait LowPriorityReadProofs {
  implicit def hlistRead2[H, T <: HList](implicit
      RH: Lazy[Read[H]],
      RT: Lazy[Read[T]]
  ): Read[H :: T] = new Read[H :: T] {
    def readPartial(a: CSV.Row): Either[Error.DecodeFailure, Either[(CSV.Row, H :: T), H :: T]] =
