...
package object fs2 {
   ...
   * Removes Empty Rows
   */
  def parseCompleteSafe[F[_]]: Pipe[
    F,
    String,
    Either[Error.ParseFailure, (CSV.Headers, Either[Error.ParseFailure, CSV.Row])]
  ] = {
    // _.through(cleanLastStringCRLF[F])
    _.through(parse1(opt(CSVParser.PERMISSIVE_CRLF) ~> CSVParser.header))
      .map[Either[Error.ParseFailure, (CSV.Headers, Stream[F, String])]] {
      ...
      .flatMap {
        _.traverse { case (h, s) =>
          s.through(parseN(opt(CSVParser.PERMISSIVE_CRLF) ~> CSVParser.record)).map { row =>
            (h, Either.right(row))
          }
        }
      }
  ...
  private def parse1[F[_], A](
      p: atto.Parser[A]
  ): Pipe[F, String, (ParseResult[A], Stream[F, String])] = s => {
    def go(
        r: ParseResult[A]
    )(s: Stream[F, String]): Pull[F, (ParseResult[A], Stream[F, String]), Unit] = {
      r match {
